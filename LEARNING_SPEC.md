...
3. **Rigorous core (when applicable).** Precise definitions, theorems, propositions, bounds, invariances. Named theorems cited, not reproved. **Proofs are not included** unless the proof technique itself is the lesson — in which case give a *short* proof (a few lines max). For everything else: state the proposition, optionally give a one-line proof sketch (e.g. "by Cauchy–Schwarz", "induction on $\ell$", "direct computation"), and either leave verification as an exercise or omit. Spelling out routine arithmetic in chapter prose is filler. When a topic is fundamentally heuristic or empirical, *say so* and skip the rigorous section rather than manufacturing a theorem.
4. **Numerical example (when it earns its place).** A numerical example is included only when **the numbers reveal something the formula does not** — a counterintuitive fact, a regime change, a non-obvious quantitative scaling, a phase transition, an asymmetry between two methods. Walking through a definition with concrete numbers is *not* a numerical example; it is a definition with arithmetic, and it is filler. If the chapter's content is fully captured by the formula plus the notebook experiments, omit this section with a one-line note ("no separate hand example — see notebook"). When included, prefer hand-computable cases ($d_k = 2$, $n = 3$) but also accept short inline code snippets when the point is a distributional / asymptotic fact.
5. **Mechanics check (when there are mechanisms to check).** A short bulleted list, one bullet per sub-component, stating *what that step does and why that step exists*. Useful for chapters with multiple moving parts (multi-head, FFN, transformer block). For single-mechanism chapters (e.g. one scaling factor, one identity), skip this section with a one-line note ("no separate mechanics check — the rigorous core is the mechanics").
6. **Exercises.** See §3.
...
- **Toy task design exposes this chapter's mechanism.** Do not reuse the same generic toy task across chapters. The notebook's task should be designed so that *what this chapter is about* is what determines whether the model solves it. A reader running the notebooks back-to-back should encounter different task shapes that each force a different mental-model update. Reused-task drift is a chapter-design failure, not a notebook efficiency win.
